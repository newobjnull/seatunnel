...
public class SFTPConnectionPool {
    ...
    public ChannelSftp connect(
        ...
        try {
            ...
            if (identityFileContent != null && identityFileContent.length() > 0) {
                if (identityFilePass != null && identityFilePass.length() > 0) {
                    jsch.addIdentity(
                            user,
                            identityFileContent.getBytes(StandardCharsets.UTF_8),
                            null,
                            identityFilePass.getBytes(StandardCharsets.UTF_8));
                } else {
                    jsch.addIdentity(
                            user, identityFileContent.getBytes(StandardCharsets.UTF_8), null, null);
                }
            }
